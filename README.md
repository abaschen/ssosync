...
## 🏗️ Architecture & Why SSO Sync?

AWS IAM Identity Center supports SCIM protocol for automatic provisioning, but currently only provides native integration with Azure AD. **SSO Sync fills this gap** by providing:

- **Google Workspace Integration**: Native support for Google Directory API
- **SCIM Compliance**: Full SCIM 2.0 protocol implementation for AWS IAM Identity Center
- **Flexible Sync Options**: Choose between group-based or user-based synchronization
...
|--------|-------------|----------|
| **`groups`** (default) | Syncs groups first, then their members | Recommended for most organizations |
| **`users_groups`** | Syncs all users first, then groups | Legacy method, useful for specific workflows |

### Architecture Overview

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│ Google          │    │ SSO Sync        │    │ AWS IAM         │
│ Workspace       │───▶│                 │───▶│ Identity Center │
│                 │    │ • CLI Tool      │    │                 │
│ • Users         │    │ • Lambda        │    │ • Users         │
│ • Groups        │    │ • SAM Deploy    │    │ • Groups        │
│ • Directory API │    │ • Dry-run       │    │ • SCIM API      │
└─────────────────┘    └─────────────────┘    └─────────────────┘
                              │
                              ▼
                       ┌─────────────────┐
...
The easiest way to deploy SSO Sync is through the AWS Serverless Application Repository:

1. **Navigate to the [SSOSync App](https://console.aws.amazon.com/lambda/home#/create/app?applicationId=arn:aws:serverlessrepo:us-east-2:004480582608:applications/SSOSync)**
2. **Configure parameters** (see Configuration section below)
3. **Deploy** - The app will create all necessary resources

### Option 2: CLI Binary

```bash
# Download the latest release
curl -L https://github.com/awslabs/ssosync/releases/latest/download/ssosync_linux_amd64.tar.gz | tar xz
